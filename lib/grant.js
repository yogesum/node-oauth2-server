...
function useAuthCodeGrant (done) {
  ...
  this.model.getAuthCode(code, function (err, authCode) {
    ...
    if (!authCode || authCode.clientId !== self.req.oauth.client.id) {
    ...
    self.req.oauth.session_id = authCode.session_id;
...
function useRefreshTokenGrant (done) {
  ...
  this.model.getRefreshToken(token, function (err, refreshToken) {
    ...
    if (!refreshToken || refreshToken.clientId !== self.req.oauth.client.id) {
    ...
    self.req.oauth.session_id = refreshToken.session_id;
...
function saveAccessToken (done) {
  ...
  this.model.saveAccessToken(accessToken, this.req.oauth.client, expires,
      this.user, this.req.oauth.session_id, function (err) {
    if (err) return done(error('server_error', false, err));
    done();
  });
}
...
function saveRefreshToken (done) {
  ...
  this.model.saveRefreshToken(refreshToken, this.req.oauth.client, expires,
      this.user, this.req.oauth.session_id, function (err) {
    if (err) return done(error('server_error', false, err));
    done();
  });
