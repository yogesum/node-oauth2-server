...
function AuthCodeGrant(config, req, res, next, check) {
  ...
  runner(fns, this, function (err) {
    if (err && res.oauthRedirect) {
      ...
      res.send(self.client.redirectUri + '?error=' + err.error +
...
function saveAuthCode (done) {
  ...
  this.model.saveAuthCode(this.authCode, this.client, expires,
...
function redirect (done) {
  this.res.send(this.client.redirectUri + '?code=' + this.authCode +
